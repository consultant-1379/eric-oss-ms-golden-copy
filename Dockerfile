...
ARG USER_NAME="eric-oss-ms-golden-copy"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-ms-golden-copy-app.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-ms-golden-copy-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-ms-golden-copy-jsb \
